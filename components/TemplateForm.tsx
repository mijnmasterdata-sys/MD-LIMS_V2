...
const TemplateForm: React.FC<TemplateFormProps> = ({ template, onSave, onCancel }) => {
  ...
  const renderHighlightedPreview = () => {
    ...
    return (
       <div 
         className="font-mono text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap leading-relaxed select-text"
    ...
    <div className="flex flex-col h-[calc(100vh-100px)] animate-in fade-in duration-300 gap-6">
      ...
      <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
        <div>
           ...
           <p className="text-sm text-gray-500 dark:text-gray-400">Upload a sample and tag fields to train the parser.</p>
        </div>
      ...
      <div className="flex-1 flex gap-6 min-h-0">
        ...
        <div className="w-1/3 flex flex-col gap-4 overflow-y-auto pr-2">
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 flex-1 flex flex-col">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Tagged Fields</h3>
                ...
                formData.mappings.map(m => (
                  <div key={m.id} className={`flex justify-between items-center p-2 rounded border ${m.color.replace('bg-', 'bg-opacity-20 ')}`}>
                    <div className="truncate pr-2">
                      ...
                      <span className="text-xs font-mono text-gray-800 dark:text-white truncate">"{m.text}"</span>
                    </div>
                    <button onClick={() => removeMapping(m.id)} className="text-gray-400 hover:text-red-500 dark:hover:text-red-400">
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
             <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">System Instruction (Auto-generated)</label>
             <textarea 
               rows={6}
               value={formData.customInstruction}
               onChange={(e) => setFormData({...formData, customInstruction: e.target.value})}
               className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-gray-900 dark:text-white font-mono text-xs focus:border-blue-500 focus:outline-none"
             />
          </div>
        ...
        <div className="w-2/3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden relative">
           ...
           <div className="p-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex justify-between items-center">
             <div className="flex items-center gap-3">
               <label className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-1.5 rounded transition-colors shadow-sm">
                 Upload Sample PDF
                 <input type="file" className="hidden" accept=".pdf" onChange={handleFileUpload} />
               </label>
               {isLoading && <span className="text-xs text-blue-600 dark:text-blue-400 animate-pulse">Processing (Scanning OCR)...</span>}
             </div>
           ...
           {selection && (
             <div className="absolute top-16 left-1/2 transform -translate-x-1/2 bg-gray-800 dark:bg-gray-900 border border-gray-600 shadow-2xl rounded-lg p-2 z-50 flex gap-1 animate-in fade-in zoom-in-95 duration-150">
                {TAG_TYPES.map(tag => (
                  <button
                    key={tag.value}
