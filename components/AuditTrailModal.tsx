...
const AuditTrailModal: React.FC<AuditTrailModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-xl w-full max-w-4xl p-6 h-[80vh] flex flex-col">
        ...
        <div className="flex-1 overflow-auto border border-gray-700 rounded bg-gray-900">
          <table className="w-full text-left text-sm text-gray-300">
            <thead className="bg-gray-800 text-xs uppercase font-medium text-gray-400 sticky top-0">
              <tr>
            ...
            <tbody className="divide-y divide-gray-800">
              ...
              {Array.from({ length: 5 }).map((_, i) => (
                <tr key={`filler-${i}`} className="hover:bg-gray-800/50 transition-colors opacity-50">
                  <td className="px-4 py-2 font-mono text-xs">2023-10-24 08:00:0{i}</td>
                  <td className="px-4 py-2"><span className="px-2 py-0.5 rounded text-xs font-semibold bg-gray-700 text-gray-300">LOGIN</span></td>
